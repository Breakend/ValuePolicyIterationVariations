...
class ValueIteration:
    ...
    def run(self, theta = 0.001):
        ...
        while True:
            ...
            for s in self.mdp.states:
                ...
                for a in self.mdp.possible_actions(s):
                    action_possibilities.append(sum([p * (r + self.mdp.gamma*V[s_prime]) for (p, s_prime) in self.mdp.T(s,a)]))
        ...
        for s in self.mdp.states:
            action_possibilities = []
            for a in self.mdp.possible_actions(s):
                action_possibilities.append((a, sum([p * (r + self.mdp.gamma*V[s_prime]) for (p, s_prime) in self.mdp.T(s,a)])))
